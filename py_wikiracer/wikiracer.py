...
def invalid_url_pattern(test_str: str):
    diss_list = []
...
def exclude_links(x: str):
    ...
    print("disallowed characters = ", disallowed_list)
...
class Parser:
    ...
    def get_links_in_page(html: str) -> List[str]:
        ...
        # "\\s*(?i)href\\s*=\\s*(\"([^\"]*\")|'[^']*'|([^'\">\\s]+))"
        # href_regex_pattern = "\\s*(?i)href\\s*=\\s*(\"([^\"]*\")|'[^']*'|([^'\">\\s]+))"
        # matches: Match = re.match(href_regex_pattern, html)
        # matches_list: [] = re.findall(href_regex_pattern, html, re.I)
        links: [] = re.findall(r'(?<=<a href=")[^"]*',  html)
        #filter(exclude_links, links)

        #disallowed_list = Internet.DISALLOWED.split(",")
        #print("disallowed characters = ", disallowed_list)

        # exclude_pattern: str = '#|:|?'
        #exclude_pattern_2 = "|".join(Internet.DISALLOWED)
        # forbidden_chars = re.compile(exclude_pattern)

        # 'Contains the word'

        clean_list = []
        for link in links:
            if invalid_url_pattern(link):
            # if forbidden_chars.search(link):
                # print("can not add this link to final clean list ", link)
                continue
            else:
                #clean_list.append(link)
                if link.startswith("/wiki") and link not in clean_list:
                    print("adding to the clean links ", link)
                    clean_list.append(link)
        ...
        print("clean links ", clean_list)
        print("# of clean links ", len(clean_list))
