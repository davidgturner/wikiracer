...
def dfs_search(src, goal) -> []:
    ...
    while S:  # and count < 3:
        ...
        if v == goal:  # Search goal node, check for our goal and if we met it return our path
            print("found goal ", v, " with a depth of ", count)
            return v
            #return prev[v]
...
source = "/wiki/Athlon_Sports" # "/wiki/Yar_Mohammad_Khan_Alakozai"  # /wiki/Fiji" # "/wiki/Calvin_Li"
goal = "/wiki/Nashville_(disambiguation)" # "/wiki/Farsi_(disambiguation)"  # "/wiki/Main_Page"  # "/wiki/Persian_language"  # "/wiki/Fiji_(disambiguation)" # "wiki/Austronesian_languages"
